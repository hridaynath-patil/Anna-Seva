...
if (usePostgres) {
  console.log('[Anna Seva] Using Supabase/PostgreSQL database.');
  const poolConfig = {
    connectionString: process.env.DATABASE_URL,
  };
  // Automatically add SSL rejectUnauthorized: false for Supabase in production/hosted environments
  ...
  // Use Render's persistent disk mount in production, local ./data otherwise
  // Can be overridden with DB_DIR_PATH environment variable
  ...
  console.log(`[Anna Seva] Using SQLite database. Path: ${DB_PATH}`);
...
export async function initDB() {
  if (usePostgres) {
    ...
    console.log('[Anna Seva] SQLite database initialized and seeded successfully.');
  }
}
